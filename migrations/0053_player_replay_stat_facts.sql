...
CREATE INDEX player_replay_stat_facts_stat_player_idx
    ...
    INCLUDE (replay_id, analysis_run_id, value, active_time_seconds, denominator_value);
...
WHERE r.canonical_analysis_run_id IS NOT NULL
  ...
  AND rp.team IN (0, 1)
GROUP BY r.canonical_analysis_run_id, rp.replay_id, rp.id, rp.platform, rp.platform_player_id, rp.team, rp.active_time_seconds
HAVING SUM(
    CASE
        WHEN COALESCE((payload.payload ->> 'active')::boolean, true)
         AND (
