 ...
 * @returns {any}
 */
export function detect_mistakes(raw_replay_data, focus_player, profile_config) {
 ...
 * @returns {any}
 */
export function list_focus_players(raw_replay_data) {
    const ret = wasm.list_focus_players(raw_replay_data);
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return takeFromExternrefTable0(ret[0]);
...
function __wbg_get_imports() {
    ...
    return {
        __proto__: null,
        "./rocket_sense_mistakes_wasm_bg.js": import0,
    };
}
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('rocket_sense_mistakes_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
